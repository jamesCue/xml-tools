...
<p:library xmlns:p="http://www.w3.org/ns/xproc" xmlns:epub="http://www.corbas.co.uk/ns/epub"
	...
	<p:documentation>
		...
		<h:p>It is important to ensure a meaningful xml:base for the package file that allows the
			...
			step it might be necessary to set one manually on the root element of the package. In
			order to keep EPUB processing sane any xml:base attribute on the root of the package
			file is stripped off before it is written to the EPUB file.</h:p>
	</p:documentation>
	...
	<p:declare-step name="write-mimetype" type="epub:write-mimetype">
		...
		<p:documentation>
			<h:p>Writes the mimetype file to disk. The only information required is the directory to
				which it should be written. Returns a <code>c:result</code> element that gives the
				full url for the file. </h:p>
		...
		<p:output port="result" primary="true">
			<p:documentation>
				<h:p>The output for the write-mimetype step is a c:result element containing the URL
					pointing to the temporary file to written. It doesn't try to write it to the
					directory containing the package file as this may not be possible.</h:p>
			</p:documentation>
		...
		<p:identity name="make-tempfile-primary">
			...
			<p:documentation>
				<h:p>Use an identity here because the output from cfx:tempfile is not primary and we
					want the context for the next step to use it so we can extract info from it with
					xpath</h:p>
			</p:documentation>
		...
		<p:store method="text" name="store-mimetype">
			<p:documentation>
				<h:p>Write out the mimetype file to disk. The value of the c:result from the
					temporary file creation is used as the URL. Result of this is a c:result
	...
	<p:declare-step name="write-container" type="epub:write-container">
		...
		<p:output port="result" primary="true">
			<p:documentation>
				<h:p>The result of the step is a c:result element containing the URL pointing to the
					temporary file containing the container file.</h:p>
			</p:documentation>
		...
		<p:template name="container-xml">
			<p:documentation>
				<h:p>The template step inserts the name of the OPF file into the full-path attribute
					of </h:p>
			</p:documentation>
			<p:input port="source">
				<p:empty/>
		...
		<p:store method="xml" name="store-container">
			<p:documentation>
				<h:p>Write out the container file to disk. The value of the c:result from the
					temporary file creation is used as the URL. Result of this is a c:result
					containing the URL which becomes the step output.</h:p>
			</p:documentation>
			<p:with-option name="href" select="//c:result"/>
	...
	<p:declare-step name="init-epub" type="epub:init-epub">
		<p:documentation>
			<h:p>Initialises the initial the EPUB structure. Writes the <h:code>mimetype</h:code>
				file and <h:code>META-INF/container.xml</h:code> file only and returns a sequence
				containing them.</h:p>
		</p:documentation>
		...
		<p:option name="opf-uri" select="'OPS/package.opf'">
			<p:documentation><h:p>Relative url for the OPF file, defaults to
						<h:code>OPS/package.opf</h:code></h:p></p:documentation>
		</p:option>
	...
	<p:declare-step name="convert-opf-to-zip-manifest" type="epub:convert-opf-to-zip-manifest">
		...
		<p:option name="content-dir" required="true">
			<p:documentation><h:p>The prefix to prepend to the manifest href to create the name
					attribute. OPF manifests are relative to the OPF file whils the zip manifest
					must be relative to EPUB root.</h:p></p:documentation>
		</p:option>
		<p:option name="opf-uri" required="true">
			<p:documentation><h:p>The URI for the package file itself is required in order to add it
					to the zipe file as the package file is not
				self-referential.</h:p></p:documentation>
		</p:option>

		<p:xslt version="2.0" name="transform-manifest">

			<p:documentation>
				<h:p>Convert the epub manifest to a zip manifest, setting the xml:base so that files
					are found in the right locations. In order to have a meaningful xml:base it must
					have either been set explicitly at some point or the OPF file must actually
					exist on disk.</h:p>
			</p:documentation>
	...
	<p:declare-step name="convert-result-to-entry" type="epub:convert-result-to-entry">
		...
		<p:documentation>
			<h:p>Given a c:result entry as source, creates a c:entry element suitable for adding to
				a zip manifest. </h:p>
		</p:documentation>
		...
		<p:option name="compression" select="'deflated'">
			<p:documentation>
				<h:p>Set the compression level to be used. Defaults to
					<h:code>deflated</h:code></h:p>
			</p:documentation>
		</p:option>
		...
		<p:template name="create-result">
			...
			<p:input port="template">
				<p:inline>
					<c:entry name="{$name}" compression-method="{$compression}" href="{//c:result}"
					/>
				</p:inline>
			</p:input>
			<p:input port="parameters">
	...
	<p:declare-step name="package-epub" type="epub:package-epub">
		<p:documentation>
			<h:p>Given an input of an OPF file, creates an EPUB file. The caller must provide the
				path to the EPUB file and the name of the directory into which content must be
				stored.</h:p>
		</p:documentation>
		...
		<p:output port="result" primary="true">
			<p:documentation>
				<h:p>The result of the step is a cx:zip-result element describing the zipe file
					created.</h:p>
			</p:documentation>
		...
		<p:option name="package-file" select="'package.opf'">
			<p:documentation><h:p> The file name for the package file. Defaults to
						<h:code>package.opf</h:code></h:p></p:documentation>
		</p:option>

		<p:option name="epub-path" required="true">
			<p:documentation><h:p>The full path to the epub-file to be
